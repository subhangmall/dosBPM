...
void setIDTHandler(uint8_t intNum, uint32_t address) {
    ...
    // kprint_hex((uint32_t)idtRequestHandlers[intNum]);
}
...
void initIDTStructures() {   
    ...
    // kprint_hex((uint32_t)idtRequestDispatchers[0]);
    // kprint_hex((uint32_t)int20);
    // kprint_hex((uint32_t)idtRequestDispatchers[0x20]);
    ...
    for (int i = 0; i < 256; i++) {
        ...
        setIDTValue(i, (uint32_t) idtRequestDispatchers[i], 0xEE, 0X08);
        // if (i == 0xFF) {
        //     kprint_hex((uint32_t)idt[i].offset1);
        //     kprint_hex((uint32_t)idt[i].offset2);
        // }
    ...
    // setIDTValue(0, (uint32_t) int0, 0x8E, 0x08);
